    ...
    return (
        <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <SectionCard title={t("companyStatus.entryPasses")}>
                    {entryPasses.length > 0 ? (
                            ...
                            {entryPasses.map((p) => (
                                <div key={p.id} className="flex items-center gap-3 bg-gray-50 dark:bg-gray-800 rounded-lg p-2.5">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <p className="text-[10px] font-mono text-gray-400 dark:text-gray-500">{p.code}</p>
                                    </div>
                                    <MiniBadge label={tBadge(p.status)} tone={statusTone(p.status)} />
                                </div>
                            ))}
                        ...
                        <>
                            <AwaitingCasto text={t("companyStatus.empty2.entryPasses")} />
                            <ContactCard moduleId="passes" team={team} />
                        </>
                    )}
                </SectionCard>

                <SectionCard title={t("companyStatus.parking")}>
                    {parkingPasses.length > 0 ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {parkingPasses.map((p) => (
                                <div key={p.id} className="flex items-center gap-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-100 dark:border-amber-500/25 rounded-lg p-2.5">
                                    <div className="bg-white rounded p-1 shrink-0">
                                        <QRCodeSVG value={`jobfair:pass:${p.code}`} size={44} fgColor="#111827" />
                                    </div>
                                    <div className="min-w-0 flex-1">
                                        <p className="text-xs font-semibold text-gray-700 dark:text-gray-300 truncate">{p.delegate}</p>
                                        <p className="text-sm font-bold text-amber-700 dark:text-amber-400 mt-0.5">{t("companyStatus.slot", { slot: p.slot || "—" })}</p>
                                        {p.location && <p className="text-[10px] text-amber-600 dark:text-amber-400">{p.location}</p>}
                        ...
                        <>
                            <AwaitingCasto text={t("companyStatus.empty2.parking")} />
                            <ContactCard moduleId="passes" team={team} />
                        </>
                    )}
                </SectionCard>
            ...
            {schedule.length > 0 && (
                <SectionCard title={t("companyStatus.eventSchedule")}>
                    <div className="flex flex-col gap-2">
                        {schedule.map((s) => (
                            <div key={s.id} className={`flex items-center gap-3 rounded-lg px-3 py-2 border ${
                                s.status === "Live" ? "border-green-200 dark:border-green-500/30 bg-green-50 dark:bg-green-500/10"
                                : s.status === "Ended" ? "border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800"
                                : "border-blue-100 dark:border-blue-500/25 bg-blue-50/50 dark:bg-blue-500/10"}`}>
                                <div className="flex flex-col items-center w-14 shrink-0">
                                    <span className="text-xs font-bold text-gray-700 dark:text-gray-300">{s.start}</span>
                                    <span className="text-[10px] text-gray-400 dark:text-gray-500">{s.end}</span>
                                </div>
                                <div className="min-w-0 flex-1">
                                    <p className="text-xs font-semibold text-gray-800 dark:text-gray-100 truncate">{s.title}</p>
                                    <p className="text-[10px] text-gray-500 dark:text-gray-400 truncate">
                                        {[s.host && t("companyStatus.host", { host: s.host }), s.location].filter(Boolean).join(" · ")}
                                    </p>
                                </div>
                                <MiniBadge label={tBadge(s.status)} tone={statusTone(s.status)} />
                            </div>
                        ))}
                    </div>
                </SectionCard>
            ...
            <SectionCard title={t("companyStatus.needHelp")}>
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">{t("companyStatus.needHelpBody")}</p>
...
const RequestsSection = ({ companyName, readOnly = false }) => {
    ...
    const existingRequests = [
        ...
        ...equipment.filter((e) => e.requestedBy).map((e) => ({ id: `eq-${e.id}`, label: `${e.qtyReq} × ${e.item}`, status: e.status === "Fulfilled" ? "Approved" : t("companyStatus.awaitingApproval"), tone: toneClass(e.status === "Fulfilled" ? "green" : "yellow") })),
    ];
    ...
    if (readOnly) {
        ...
        return (
            <div className="flex flex-col gap-3">
                <SectionCard title={t("companyStatus.requestsSubmitted")}>
                        ...
                        <div className="flex flex-col gap-1.5">
                            {existingRequests.map((r) => (
                                <div key={r.id} className="flex items-center justify-between gap-2 text-xs bg-gray-50 dark:bg-gray-800 rounded-lg px-2.5 py-1.5">
                                    ...
                                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold whitespace-nowrap ${r.tone}`}>{tBadge(r.status)}</span>
                                </div>
                            ))}
                        </div>
                    ) : <p className="text-xs text-gray-400 dark:text-gray-500">{t("companyStatus.noRequestsYet")}</p>}
                </SectionCard>
            </div>
        );
    }
...
const STATUS_CONFIG = {
    Confirmed: {
        card: 'bg-green-50 dark:bg-green-500/10 border-green-200 dark:border-green-500/25', text: 'text-green-800 dark:text-green-300', sub: 'text-green-600 dark:text-green-400',
        badge: 'bg-green-100 dark:bg-green-500/15 text-green-700 dark:text-green-300 border-green-200 dark:border-green-500/25',
        icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>),
    },
    Pending: {
        card: 'bg-yellow-50 dark:bg-amber-500/10 border-yellow-200 dark:border-amber-500/25', text: 'text-yellow-800 dark:text-amber-300', sub: 'text-yellow-600 dark:text-amber-400',
        badge: 'bg-yellow-100 dark:bg-amber-500/15 text-yellow-700 dark:text-amber-300 border-yellow-200 dark:border-amber-500/25',
        icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>),
    },
    Canceled: {
        card: 'bg-red-50 dark:bg-red-500/10 border-red-200 dark:border-red-500/25', text: 'text-red-800 dark:text-red-300', sub: 'text-red-600 dark:text-red-400',
        badge: 'bg-red-100 dark:bg-red-500/15 text-red-700 dark:text-red-300 border-red-200 dark:border-red-500/25',
...
const CompanyStatus = ({ viewCompanyId, readOnly = false }) => {
    ...
    const STATUS_TABS = [t("companyStatus.tabs.overview"), t("companyStatus.tabs.eventDay"), t("companyStatus.tabs.requests")];
    ...
    useEffect(() => {
        const fetchCompanyData = async () => {
            try {
                ...
                setError(t("companyStatus.loadFailed"));
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleConfirmAttendance = async () => {
        try {
            ...
            setConfirmError(t("companyStatus.confirmFailed"));
        } finally {
        ...
        <div className="flex-1 flex flex-col gap-3 md:gap-4 overflow-y-auto min-h-0 p-3 md:p-4 animate-fadeIn">
            {readOnly && (
                <div className="flex items-center gap-2 bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/25 rounded-lg px-3 py-2 text-xs text-blue-700 dark:text-blue-300 font-medium">
                    ...
                    {t("companyStatus.previewMode", { company: companyData?.companyName || t("companyStatus.previewModeFallback") })}
                </div>
            )}
            <div className="bg-white dark:bg-gray-800 rounded-lg p-3 md:p-4 border border-gray-100 dark:border-gray-700 shadow-sm">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0">
                            <h1 className="text-sm md:text-base font-bold text-gray-800 dark:text-gray-100 truncate">{companyData?.companyName}</h1>
                            <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{companyData?.sector} · {companyData?.city}</p>
                        </div>
                    </div>
                    <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-xs shrink-0 ${cfg.badge}`}>
                        {cfg.icon}<span className="font-semibold">{t(`companyStatus.status.${status}`, status)}</span>
                    </div>
                </div>
            </div>
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                        <p className={`text-sm font-semibold ${cfg.text} mb-1`}>
                            {t(`companyStatus.statusHeadline.${status}`, status)}
                        </p>
                        <p className={`text-xs ${cfg.sub}`}>{t(`companyStatus.statusMessage.${status}`, status)}</p>
                        {status === 'Pending' && !readOnly && (
                            <div className="mt-2.5">
                                <button onClick={handleConfirmAttendance} disabled={isConfirming}
                                    ...
                                    {isConfirming ? (<><svg className="animate-spin w-3 h-3" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>{t("companyStatus.confirming")}</>) : (<><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>{t("companyStatus.confirmMyAttendance")}</>)}
                                </button>
                                {confirmError && <p className="mt-1.5 text-xs text-red-600 dark:text-red-400">{confirmError}</p>}
                            </div>
                        )}
                    </div>
                </div>
            ...
            {activeTab === 0 && (
                <>
                    <div className="grid grid-cols-3 gap-3">
                        <StatCard label={t("companyStatus.totalApplicants")} value={applicantsCount}
                            icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>} />
                        <StatCard label={t("companyStatus.openPositions")} value={companyData?.noOfPositions || 0} iconBg="bg-purple-100 dark:bg-purple-500/15" iconColor="text-purple-600 dark:text-purple-300"
                            icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>} />
                        <StatCard label={t("companyStatus.representatives")} value={reps.length} iconBg="bg-blue-100 dark:bg-blue-500/15" iconColor="text-blue-600 dark:text-blue-300"
                            icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>} />
                    </div>

                    {!readOnly && (
                        <div className="flex items-center justify-between bg-white dark:bg-gray-800 rounded-lg p-3 md:p-4 border border-gray-100 dark:border-gray-700 shadow-sm">
                            <div>
                                <p className="text-sm font-semibold text-gray-800 dark:text-gray-100">{t("companyStatus.manageSettings.title")}</p>
                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{t("companyStatus.manageSettings.subtitle")}</p>
                            </div>
                            <a href="/company-settings" className="text-xs font-semibold text-white rounded-lg px-3.5 py-2 hover:opacity-90 transition-opacity shrink-0" style={{ background: "#0E7F41" }}>
                                {t("companyStatus.manageSettings.cta")}
                            </a>
                        </div>
                    )}

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <SectionCard title={t("companyStatus.contactInformation")}>
                            <div className="flex items-center gap-2 text-xs text-gray-700 dark:text-gray-300">
                                <svg className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                                <a href={`mailto:${companyData?.email}`} className="text-blue-600 dark:text-blue-400 hover:underline truncate">{companyData?.email}</a>
                            </div>
                        </SectionCard>
                        <SectionCard title={t("companyStatus.representatives")}>
                            {reps.length > 0 ? (
                                <div className="flex flex-col gap-1.5">
                                    {reps.map((rep, i) => (
                                        <div key={i} className="flex items-center gap-2 text-xs text-gray-700 dark:text-gray-300">
                                            <span className="w-5 h-5 rounded-full bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-300 text-[10px] font-semibold flex items-center justify-center shrink-0">{rep.trim().charAt(0).toUpperCase()}</span>
                                            <span className="truncate">{rep.trim()}</span>
                                        </div>
                                    ))}
                                </div>
                            ) : <p className="text-xs text-gray-400 dark:text-gray-500">{t("companyStatus.noneListed")}</p>}
                        </SectionCard>
                        <SectionCard title={t("companyStatus.industryFields")}>
                            <div className="flex flex-wrap gap-1.5">{fields.map((f, i) => <TagPill key={i} label={f} variant="cyan" />)}</div>
                        </SectionCard>
                        <SectionCard title={t("companyStatus.opportunityTypes")}>
                            <div className="flex flex-wrap gap-1.5">
                                {companyData?.opportunityTypes?.length > 0
                                    ? companyData.opportunityTypes.map((ot, i) => <TagPill key={i} label={ot} variant="purple" />)
                                    : <span className="text-xs text-gray-400 dark:text-gray-500">{t("companyStatus.notSpecified")}</span>}
                            </div>
                        </SectionCard>
                        {companyData?.preferredMajors?.length > 0 && (
                            <SectionCard title={t("companyStatus.preferredMajors")} className="md:col-span-2">
                        ...
                        {companyData?.preferredQualities && (
                            <SectionCard title={t("companyStatus.idealCandidateQualities")} className="md:col-span-2">
                                <p className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed">{companyData.preferredQualities}</p>
                            </SectionCard>
                        )}
                    </div>
                </>
            ...
            {companyData?.surveyResult?.length > 0 && (
                <div className="bg-green-50 dark:bg-green-500/10 rounded-lg p-3 border border-green-200 dark:border-green-500/25 flex items-center gap-3">
                    <div className="p-1.5 bg-green-100 dark:bg-green-500/20 rounded-full shrink-0"><svg className="w-3.5 h-3.5 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></div>
                    <div><p className="text-xs font-medium text-green-800 dark:text-green-300">{t("companyStatus.surveyCompleted")}</p><p className="text-[10px] text-green-600 dark:text-green-400">{t("companyStatus.surveyThankYou")}</p></div>
                </div>
            )}
        </div>
    );
