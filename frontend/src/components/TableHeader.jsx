...
const SortArrow = ({ isActive, isAscending }) => (
    <svg
        className={`w-3 h-3 ms-1 transition-transform duration-200 ${isActive ? 'opacity-100' : 'opacity-40'} ${isActive && !isAscending ? 'rotate-180' : 'rotate-0'}`}
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
    >
...
const TableHeader = ({userType, sortColumn, isAscending, onSort}) => {
    ...
    ?
        <div className="row grid py-1.5 pl-3 md:pl-5 pr-4 md:pr-6 rounded-lg font-semibold text-[11px] xl:text-xs items-center">
            ...
            <h2 className={`${getClickableClass('Nationality')} ${hideOnTablet}`} onClick={() => onSort?.('Nationality')}><span>{t('applicants.columns.nationality')}</span><SortArrow isActive={sortColumn === 'Nationality'} isAscending={isAscending} /></h2>
            <h2 className={`${getClickableClass('CGPA')} ${hideOnTablet}`} onClick={() => onSort?.('CGPA')}><span>{t('applicants.columns.cgpa')}</span><SortArrow isActive={sortColumn === 'CGPA'} isAscending={isAscending} /></h2>
    ...
    :
        <div className="row-manager grid py-1.5 pl-3 md:pl-5 pr-4 md:pr-6 rounded-lg font-semibold text-[11px] xl:text-xs items-center">
            ...
            <h2 className={`${getClickableClass('Representatives')} ${hideOnTablet}`} onClick={() => onSort?.('Representatives')}><span>{t('managersCols.reps')}</span><SortArrow isActive={sortColumn === 'Representatives'} isAscending={isAscending} /></h2>
            <h2 className={`${getClickableClass('City')} ${hideOnTablet}`} onClick={() => onSort?.('City')}><span>{t('managersCols.city')}</span><SortArrow isActive={sortColumn === 'City'} isAscending={isAscending} /></h2>
