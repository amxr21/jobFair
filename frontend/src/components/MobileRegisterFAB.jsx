...
const MobileRegisterFAB = () => {
    ...
    return (
        <>
            ...
            <Modal visible={isModalVisible} onClose={closeModal} maxWidth="max-w-[320px]" wrapperClassName="md:hidden" contentClassName="p-4">
                <div className="flex items-center justify-between mb-3">
                    <p className="text-sm font-semibold">
                        {isCameraOn ? t("common.fab.registerApplicant") : t("common.fab.confirmAttendance")}
                    </p>
                    <button
                        onClick={closeModal}
            ...
            {showMenu && (
                <div className="md:hidden fixed bottom-32 end-4 z-[9998] flex flex-col gap-2 animate-fadeIn">
                    <button
                        onClick={handleRegister}
                        className="flex items-center gap-2 bg-surface-card text-primary border border-primary px-4 py-2.5 rounded-full shadow-lg text-sm font-medium"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 4.875c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v4.5c0 .621-.504 1.125-1.125 1.125h-4.5A1.125 1.125 0 013.75 9.375v-4.5zM3.75 14.625c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v4.5c0 .621-.504 1.125-1.125 1.125h-4.5a1.125 1.125 0 01-1.125-1.125v-4.5zM13.5 4.875c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v4.5c0 .621-.504 1.125-1.125 1.125h-4.5A1.125 1.125 0 0113.5 9.375v-4.5z" />
                        </svg>
                        {t("common.fab.registerApplicant")}
                    </button>
                    {isCASTO && (
                        <button
                            onClick={handleConfirm}
                            className="flex items-center gap-2 bg-primary text-primary-contrast px-4 py-2.5 rounded-full shadow-lg text-sm font-medium"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            {t("common.fab.confirmAttendance")}
                        </button>
                    )}
                </div>
            )}
            ...
            <button
                ...
                aria-label={showMenu ? t("common.close") : t("common.fab.actions")}
                className={`md:hidden fixed bottom-20 end-4 z-[9998] w-14 h-14 rounded-full shadow-lg flex items-center justify-center transition-all duration-300 ${
                    showMenu
                        ? 'bg-gray-600 rotate-45'
                        : 'bg-primary'
                }`}
            >
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className={`w-6 h-6 ${showMenu ? 'text-white' : 'text-primary-contrast'}`}>
