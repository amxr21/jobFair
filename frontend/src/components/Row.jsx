...
const ApplicantModal = ({visible, onClose, children}) => {
    ...
    if(email == 'casto@sharjah.ac.ae'){ number -= 1; return '';}
        ...
        ?
            <div id={ticketId} className={`row relative overflow-hidden grid py-4 pl-7 pr-14 min-h-24 h-full ${flags?.includes(user?.companyName) ? "border border-2 border-green-500 bg-white" :'bg-white'} ${shortlistedBy?.length ? 'border border-blue-400' : rejectedBy?.length ? 'border border-red-400' : ''} rounded-xl items-center mb-2 text-sm xl:text-base`}>
