    ...
    return (
        <div className="relative w-full h-2 shrink-0">
            ...
            <div
                onMouseEnter={enter}
                onMouseLeave={leave}
                className={`absolute left-0 right-0 top-2 z-20 transition-all duration-150 bg-white rounded-md shadow-lg ${
...
const PageContainer = ({ user, children, title, titleExtra, headerRight, showAccessButtons = false, collapsibleTopBar = false }) => {
    return (
        <div className="page-container flex flex-col gap-y-2 md:gap-y-2 flex-1 min-w-0 overflow-y-auto p-2 md:p-4 lg:p-0 max-h-[95vh]">
            ...
            <div id="Hero" className="bg-[#F3F6FF] flex flex-col grow overflow-hidden rounded-lg p-2 md:p-3 lg:p-4 w-full animate-fadeIn">

                {/* Header Section */}
                {(title || titleExtra || headerRight) && (
                    <div className="flex md:flex-row flex-col justify-between items-start md:items-center gap-1.5 md:gap-0 pl-1 border-b border-b-gray-300 pb-2 md:pb-2.5 mb-1.5 md:mb-2">
                        <div className="flex flex-col md:flex-row gap-1.5 md:gap-x-3 items-start md:items-center w-full md:w-auto">
                            <div className="flex items-center gap-2">
                                {title && <h2 className="text-sm md:text-base xl:text-lg font-bold">{title}</h2>}
                                <PageHelp user={user} />
                            </div>
                            {titleExtra}
                        </div>
