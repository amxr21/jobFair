...
const ToolbarButton = ({ onClick, title, children }) => (
    <button
        ...
        className="w-8 h-8 rounded-md border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center justify-center transition-colors text-sm"
    >
        {children}
    </button>
);
        ...
        <Modal visible={visible} onClose={onClose} maxWidth="max-w-lg" zIndex={100000}>
            <div className="bg-[#0E7F41] text-white px-5 py-4 flex items-center justify-between shrink-0">
                <div className="min-w-0">
                    <h2 className="text-base font-bold truncate">{t("emailCompose.titlePrefix", { company: companyName || t("emailCompose.fallbackCompany") })}</h2>
                    <p className="text-xs text-white/80 truncate">
                        {t("emailCompose.subtitle")} <span className="bidi-ltr">{to}</span>
                    </p>
                </div>
                <button onClick={onClose} className="p-2 hover:bg-white/20 rounded-lg transition-colors shrink-0" aria-label={t("emailCompose.close")}>
            ...
            <div className="p-5 flex flex-col gap-3 overflow-y-auto">
                <div className="flex flex-col gap-1">
                    <label className="text-xs font-medium text-gray-500 dark:text-gray-400">{t("emailCompose.subject")}</label>
                    <input
                        value={subject}
                        onChange={(e) => setSubject(e.target.value)}
                        placeholder={t("emailCompose.subjectPlaceholder")}
                        className="border border-gray-200 dark:border-gray-600 bg-surface-card text-fg rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-green-500"
                    />
                </div>

                <div className="flex flex-col gap-1">
                    <label className="text-xs font-medium text-gray-500 dark:text-gray-400">{t("emailCompose.message")}</label>
                    <div className="flex items-center gap-1.5 mb-1.5">
                        <ToolbarButton title={t("emailCompose.bold")} onClick={() => exec("bold")}><span className="font-bold">B</span></ToolbarButton>
                        <ToolbarButton title={t("emailCompose.bulletList")} onClick={() => exec("insertUnorderedList")}>
                        ...
                        <ToolbarButton title={t("emailCompose.numberedList")} onClick={() => exec("insertOrderedList")}>
                            <span className="text-xs font-semibold">1.</span>
                        </ToolbarButton>
                        <span className="text-[10px] text-gray-400 dark:text-gray-500 ms-1">{t("emailCompose.formattingNote")}</span>
                    </div>
                    <div
                        ref={editorRef}
                        contentEditable
                        suppressContentEditableWarning
                        data-placeholder={t("emailCompose.writePlaceholder")}
                        className="min-h-[140px] max-h-[40vh] overflow-y-auto border border-gray-200 dark:border-gray-600 text-fg rounded-lg px-3 py-2 text-sm leading-relaxed focus:outline-none focus:ring-1 focus:ring-green-500 email-compose-editor"
                    />
                </div>

                <div className="flex justify-end gap-2 pt-1">
                    <button onClick={onClose} className="text-xs font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 px-3 py-2">{t("emailCompose.cancel")}</button>
